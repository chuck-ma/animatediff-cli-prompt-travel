...
def create_config(
    org_movie: Annotated[
        Path,
        typer.Argument(
            path_type=Path, file_okay=True, dir_okay=False, exists=True, help="Path to movie file"
        ),
    ] = ...,
...
def generate(
    stylize_dir: Annotated[
        Path,
        typer.Argument(
            path_type=Path, file_okay=False, dir_okay=True, exists=True, help="Path to stylize dir"
        ),
    ] = ...,
    length: Annotated[
        int,
    ...
    model_config.stylize_config["0"]["length"] = min(
        model_config.stylize_config["0"]["length"] - frame_offset, length
    )
    ...
    if frame_offset > 0:
        ...
        model_config.controlnet_map["input_image_dir"] = os.path.relpath(
            new_controlnet_img_dir.absolute(), data_dir
        )
    ...
    model_config.steps = (
        model_config.stylize_config["1"]["steps"]
        if "steps" in model_config.stylize_config["1"]
        else model_config.steps
    )
    ...
    if model_config.img2img_map["enable"]:
        model_config.img2img_map["init_img_dir"] = os.path.relpath(
            Path(output_0_img_dir).absolute(), data_dir
        )
...
def interpolate(
    ...
    try:
        ...
        logger.info(
            f"Please find the installation method of cupy for your CUDA version from the following URL"
        )
    ...
    for sty1, sty2 in tqdm(zip(stylize_frame, stylize_frame[1:]), total=len(stylize_frame[1:])):
        ...
        shutil.copy(
            frame_dir.joinpath(f"{head:08d}.png"),
            output_dir.joinpath(f"{head*interpolation_multiplier:08d}.png"),
        )
    ...
    save_output(
        out_images, output_dir, out_file, model_config.output, True, save_frames=None, save_video=None
    )

    out_file = save_dir.joinpath(f"00_original")
    save_output(
        out_images, org_frame_dir, out_file, model_config.output, True, save_frames=None, save_video=None
    )
...
def create_mask(
    ...
    for output, size in output_list:
        ...
        model_config.img2img_map["init_img_dir"] = os.path.relpath(
            (output / "00_img2img").absolute(), data_dir
        )
